...
if page == "🏡 Home":
    ...
    st.subheader("📊 Your Productivity Progress")
    progress = np.random.randint(50, 100)
    st.progress(progress)
    st.write(f"Your productivity level today is at {progress}%! Keep pushing forward!")
...
# Habit Tracker
    ...
    st.subheader("📈 Habit Consistency Over Time")
    habit_days = list(range(1, days + 1))
    habit_progress = np.cumsum(np.random.randint(1, 4, size=days))
    plt.plot(habit_days, habit_progress, marker='o', linestyle='-', color='green')
    plt.xlabel("Days")
    plt.ylabel("Effort Level")
    plt.title("Your Habit Building Journey")
    st.pyplot(plt)
...
# Daily Motivation
    ...
    st.image("https://source.unsplash.com/600x400/?motivation,inspiration", use_container_width=True)
...
# Inspirational Stories
    ...
    for name, story in stories:
        st.subheader(name)
        st.write(story)
        st.image(f"https://source.unsplash.com/600x400/?success,achievement", use_container_width=True)
...
# Productivity Tips
    ...
    st.write(f"💡 **Tip for Today:** {tips[date.today().day % len(tips)]}")
    st.image("https://source.unsplash.com/600x400/?productivity,work", use_container_width=True)
...
# Growth Mindset
    ...
    A growth mindset is the belief that abilities and intelligence can be developed with effort, learning, and persistence.
    """)
    st.image("https://source.unsplash.com/600x400/?learning,growth", use_container_width=True)
