...
st.sidebar.title("🧠 Growth Mindset Hub Navigation")
st.sidebar.header("Quick Access")
st.sidebar.markdown("[Home](#welcome-to-the-growth-mindset-hub)")
st.sidebar.markdown("[Quiz](#growth-mindset-quiz)")
st.sidebar.markdown("[Challenge](#daily-growth-mindset-challenge)")
st.sidebar.markdown("[Quotes](#inspirational-quotes)")
...
score = 0
for q in questions:
    answer = st.radio(q, ["Strongly Disagree", "Disagree", "Neutral", "Agree", "Strongly Agree"], key=q)
    if answer in ["Agree", "Strongly Agree"]:
        score += 1
...
# Add a footer
st.markdown("---")
st.markdown("Built with ❤️ using Streamlit | © 2025 Growth Mindset Hub")
